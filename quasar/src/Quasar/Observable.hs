...
class IsObservable r a | a -> r where
  ...
  -- data.
  attachObserver :: a -> ObserverCallback r -> ObserverContext -> STMc '[] TSimpleDisposer
...
observe observable callbackFn = do
  ...
  disposer <- liftSTMc $ attachObserver observable wrappedCallback ctx
...
registerObserver :: ObserverRegistry a -> ObserverCallback a -> ObserverContext -> ObservableState a -> STMc '[] TSimpleDisposer
