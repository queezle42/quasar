...
observe
  :: (MonadQuasar m, MonadSTM' r CanThrow m)
  => Observable a
  -> (ObservableState a -> STM' NoRetry CanThrow ()) -- ^ callback
  -> m Disposer
observe observable callbackFn = do
  ...
  liftSTM' do
    ...
    attachResult <- noRetry $ noThrow $ attachObserver observable (observableCallback \x -> callbackFn x `catchAllSTM'` throwToExceptionSink sink) ctx
...
data ObserverContext = ObserverContext ExceptionSink ResourceManager
...
newtype ObserverCallback a = ObserverCallback (ObservableState a -> STM' NoRetry NoThrow ())

observableCallback :: (ObservableState a -> STM' NoRetry NoThrow ()) -> ObserverCallback a
observableCallback = ObserverCallback

execObservableCallback :: MonadSTM' r t m => ObserverCallback a -> ObservableState a -> m ()
execObservableCallback (ObserverCallback fn) arg = noRetry $ noThrow $ fn arg
...
instance IsObservable a (ConstObservable a) where
  attachObserver (ConstObservable state) callback _ = do
    execObservableCallback callback state
    pure $ pure mempty
...
instance IsObservable a (LiftA2Observable a) where
  attachObserver (LiftA2Observable fn fx fy) callback ctx = do
    ...
    let callCallback = do
          ...
          mapM_ (execObservableCallback callback) mergedValue
    dx <- attachObserver fx (observableCallback \update -> writeTVar var0 (Just update) >> callCallback) ctx
    dy <- attachObserver fy (observableCallback \update -> writeTVar var1 (Just update) >> callCallback) ctx
    pure $ dx <> dy
...
newtype ObserverRegistry a = ObserverRegistry (TVar (HM.HashMap Unique (ObservableState a -> STM' NoRetry NoThrow ())))
...
registerObserver (ObserverRegistry var) callback (ObserverContext _ rm) currentState = do
  key <- newUniqueSTM
  let execCallbackFn = execObservableCallback callback
  ...
  tryAttachResource rm disposer >>= \case
    Right () -> do
      noRetry $ noThrow $ execCallbackFn currentState
      pure $ Right disposer
    Left ex -> pure $ Left ex
...
instance IsObservable a (ObservableVar a) where
  attachObserver (ObservableVar var registry) callback context =
    registerObserver registry callback context . ObservableValue =<< readTVar var
