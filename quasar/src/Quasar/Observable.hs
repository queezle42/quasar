...
type ObserverCallback a = ObservableState a -> STM' NoRetry NoThrow ()
...
instance IsObservable a (MappedObservable a) where
  attachObserver (MappedObservable fn observable) callback sink = attachObserver observable (callback . fmap fn) sink
...
instance IsObservable a (LiftA2Observable a) where
  attachObserver (LiftA2Observable fn fx fy) callback ctx = do
    ...
    dx <- attachObserver fx (\update -> writeTVar var0 (Just update) >> callCallback) ctx
    dy <- attachObserver fy (\update -> writeTVar var1 (Just update) >> callCallback) ctx
    pure $ dx <> dy
